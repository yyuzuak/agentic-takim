...
> **Durum:** v2.3 Live Preview **tamamlandı ve canlı doğrulandı** — atoms.dev "uygulamayı canlı gör" anı. Sistem artık üretilen uygulamayı **tarayıcıda çalışır halde gösteriyor**: bağımsız `services/preview` (port 8005, tek-slot in-memory, non-root, read-only workspace→/tmp) `npm install`+`prisma db push`+seed+`npm run dev` çalıştırıp dev server'ı ayakta tutar; kullanıcı **localhost:8100**'ü açar (reverse-proxy yok → Next HMR sorunsuz), TTL auto-stop. UI'da "Önizle" → status poll → "Uygulamayı Aç" linki. Canlı doğrulandı: üretilen blog uygulaması `localhost:8100`'de açıldı + `/api/posts` gerçek SQLite verisi döndürdü. **Tam döngü: prompt → ajanlar → assemble → validate → build → çalışan uygulama.** Sıradaki: v3.0 Autonomous Repair.
...
| `services/sandbox` | Sandbox (port 8004) — Build Execution: gerçek npm install/prisma/build, structured errors (stateless, non-root) |
| `services/preview` | Preview (port 8005, app 8100) — Live Preview: canlı `npm run dev`, tek-slot, TTL auto-stop |
...
- [x] **v2.0-A Real Agents** (canlı doğrulandı, gpt-4o) — agent-runner stub `_collaborate()` → gerçek LiteLLM reasoning; skill→prompt registry (Mimar/Usta/Pusula… uzman sistem prompt'ları), upstream artifact context passing, producer/critic/synthesizer rolleri; **test-mode gating** (fail_*/stable_output/scores → deterministik stub, mevcut retry/DLQ acceptance'ları korunur) + her LLM hatasında graceful stub fallback; `GET /tasks/{id}/artifacts` + Studio'da Artifacts paneli (markdown/kod render); `scripts/verify_real_agents.sh` (RA1–RA5: gerçek mimari/kod/context-passing + A7 regresyon → yeşil). Faz haritası: v2.0-A → v2.0-B Artifact Pipeline → v2.1 Workspace → v2.2 Build Sandbox → v2.3 Live Preview → v3.0 Self-Improving
- [x] **v2.0-B App Builder** (canlı doğrulandı) — artifact → çalıştırılabilir repo. Stack: Next.js (App Router, TS) + **Prisma + SQLite** (zero-infra, tek komut). 5 katman: Workspace (scaffold `config/stacks/nextjs-prisma-sqlite/`) + File Placement (namespace ownership) + **2-faz Dependency Synthesizer** (import scan + rule-based resolver) + Entry Composer (file-routing → minimal) + **STRICT Build Validator** (`scripts/build_validator.py`: import graph, prisma id/relation, route→model, fetch→endpoint). App-build DAG deterministik (LLM bypass), düğüm içeriği gerçek LLM. `scripts/assemble_repo.py` → `npm install && prisma db push && npm run dev` → çalışan app. B1–B10 (cold-start dahil) yeşil
- [x] **v2.1 Workspace Runtime** (canlı doğrulandı) — `services/builder` (port 8003): assemble+validate+persist runtime'a taşındı, UI'dan tek-tık (`POST /tasks/{id}/build`); build = task'tan türeyen kaynak (Build #N); kimlik modeli `build_fingerprint`(içerik, cross-task cache)/`build_id`/`build_number`; `.build_manifest.json` dosya envanteri (path+sha256+size); DAG snapshot + validator versioning; idempotent dedupe (task_id+fingerprint); builds tablosu (migration 0012); UI **Workspace Explorer** (Build kartı + dosya ağacı + içerik). Sandbox/exec builder'a girmedi (v2.2 ayrı servis). C1–C8 yeşil
- [x] **v2.2 Build Execution** (canlı doğrulandı) — `services/sandbox` (port 8004, stateless, non-root, read-only workspace→/tmp kopya, mem/cpu limit): gerçek `npm install`+`prisma db push`+`npm run build`; **structured error extraction** (`{phase, category, file, message, severity}`, ham log fallback); `build_runs` tablosu (migration 0013); control-plane proxy+persist; UI BuildCard "Çalıştır" → build raporu. Canlı: blog uygulaması uçtan uca build (25s); kasıtlı hata → failed @ build + structured error. D1–D8 yeşil. Closed-loop repair → v3.0 (kasıtlı dışarıda)
