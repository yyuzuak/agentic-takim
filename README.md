...
> **Durum:** v2.2 Build Execution **tamamlandı ve canlı doğrulandı**. Sistem artık üretilen repo'yu **gerçekten build ediyor**: bağımsız `services/sandbox` (port 8004, stateless, non-root, read-only workspace → /tmp kopya) gerçek `npm install` + `npx prisma db push` + `npm run build` çalıştırır; **structured error extraction** (`{phase, category, file, message, severity}` — ham log değil) + aşama bazlı sonuç `build_runs`'a yazılır; UI'da "Çalıştır" → build raporu (aşama rozetleri + yapılandırılmış hata + log). Canlı: üretilen blog uygulaması uçtan uca build oldu (install+prisma+build ✓, 25s); kasıtlı bozuk kod → failed @ build + doğru structured error. "Repo doğru mu?" (v2.1) → **"repo çalışıyor mu?" (v2.2)**. Sıradaki: v2.3 Live Preview / v3.0 Autonomous Repair.
...
- [ ] v2.3 Live Preview — build sonrası `npm run dev` + proxy'lenmiş çalışan URL (atoms.dev deneyimi)
