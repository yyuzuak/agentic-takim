...
> **Durum:** v2.0-A Real Agents **tamamlandı ve canlı doğrulandı** (gpt-4o). agent-runner'daki stub `_collaborate()` gerçek LiteLLM çağrısına dönüştürüldü; ajanlar artık skill'e özel prompt'larla gerçek artifact üretiyor — Mimar gerçek teknik tasarım (teknoloji yığını + bileşen diyagramı + kararlar), Usta gerçek kod (çok-dosyalı), context passing (Mimar→Usta) çalışıyor. `scripts/verify_real_agents.sh` (RA1–RA5 + A7 regresyon) yeşil. Test-mode gating + graceful fallback mevcut tüm garantileri korur. Sıradaki: v2.0-B App Builder (artifact → workspace → dosya → build → preview).
...
- [ ] v2.0-B Artifact Pipeline — app-builder ajan/şablonları (frontend/backend/db/devops), workspace soyutlaması, dosya yazma
