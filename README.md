...
> **Durum:** v2.0-B App Builder **tamamlandı ve canlı doğrulandı**. Sistem artık "kod" değil **çalıştırılabilir repo** üretiyor: `Basit bir blog uygulaması yap` → 5-düğümlü app-build DAG → ajanlar Next.js+Prisma şekilli dosyalar üretir → **deterministik Project Assembler** (scaffold + 2-faz dependency synthesis + tek-kaynak prisma şema) → **STRICT Build Validator** (import graph + route→model + fetch→endpoint) → `npm install && prisma db push && npm run dev` → **çalışan uygulama** (`/api/posts` gerçek SQLite verisi döndürüyor). B1–B10 yeşil (cold-start dahil). Mimari: LLM = semantic compiler frontend, sistem = deterministic backend gate. Sıradaki: v2.1 Workspace Runtime.
...
- [ ] v2.1 Workspace Runtime — assembly+validator'ı runtime'a taşı (DAG içi dosya yazımı, UI'dan tek-tık üret), çoklu-stack (FastAPI)
- [ ] v2.2 Build Sandbox — izole konteynerde otomatik `npm install && build`; hata → ajana feedback loop
