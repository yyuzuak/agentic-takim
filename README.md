...
> **Durum:** v2.1 Workspace Runtime **tamamlandı ve canlı doğrulandı**. App Builder artık host CLI değil **runtime servisi**: bağımsız `services/builder` (port 8003) UI'dan tek tıkla (`POST /tasks/{id}/build`) artifact'ları assemble+validate edip kalıcı **workspace**'e yazar ve **izlenebilir build kaydı** üretir. Build artık task'tan türeyen bir **kaynak** (Task → Build #1/#2/…); kimlik modeli `build_fingerprint` (içerik, cross-task cache) / `build_id` / `build_number`; `.build_manifest.json` dosya envanteri (path+sha256+size → tree/diff/cache temeli); DAG snapshot + validator versioning. UI **Workspace Explorer**: Build kartı, validator rozeti+sürüm, dosya ağacı + içerik görüntüleyici. C1–C8 yeşil (dedupe/reproducibility dahil). Sıradaki: v2.2 Build Sandbox (gerçek `npm install/build`).
...
- [ ] v2.2 Build Sandbox — izole `services/sandbox`: gerçek `npm install && build && prisma db push`; hata → ajana feedback loop
