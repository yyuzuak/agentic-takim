...
```
`type: "research"` veya hedefte "araştır/analiz" geçmesi araştırma şablonunu seçer. Intent parsing şimdilik kural tabanlıdır (LLM tabanlısı roadmap'te).
...
- [x] Kaptan: intent parsing + task decomposition (çok-adımlı DAG; paralel + join + bağımlılık)
      — şimdilik **kural tabanlı**; LLM tabanlı `niyet-ayristirma` aşağıda
- [ ] LLM tabanlı intent parsing + decomposition (LiteLLM üzerinden)
